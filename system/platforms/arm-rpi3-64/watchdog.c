 ...
 *      OK
 */
syscall watchdogset(uint32_t msecs)
{
    *(volatile uint32_t *)PM_WDOG = PM_PASSWORD | (msecs * PM_WDOG_UNITS_PER_MILLISECOND);
    uint32_t cur = *(volatile uint32_t *)PM_RSTC;
    *(volatile uint32_t *)PM_RSTC = PM_PASSWORD | (cur & PM_RSTC_WRCFG_CLR) |
