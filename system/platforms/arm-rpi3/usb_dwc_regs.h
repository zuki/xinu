...
#define DWC_SOFT_RESET      (1 << 0)
     ...
     * @name ホストモードレジスタ
     *
     * このポイント(0x400)から始まるレジスタは「ホストモード」レジスタである.
     * これらはOTG（On-The-Go）プロトコルの「ホストモード」パートで使用される。
     * OTGはこのハードウェアがUSBホストまたはUSBデバイスとして動作することを
     * 可能にしている。このドライバではホストモードだけを扱うのでデバイスに
     * 対するレジスタは宣言しない
     */
