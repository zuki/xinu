...
static const struct {
    ...
    uint nargs;
    uint64_t args[4]; /* Assumes all args are 8 bytes. */
} fprintf_specs[] = {
    { /* Empty string */
        .format = "",
        .expected_output = "",
    ...
    { /* Large max width */
        ...
        .nargs = 1,
        .args = {(uint64_t)"abcdefghijklmnopqrstuvwxyz"},
    },
    { /* Zero */
        .format = "%d",
        .expected_output = "0",
    ...
    { /* Combination of different conversion specifiers */
        ...
        .nargs = 4,
        .args = {4000, (uint64_t)"foobar", 0xfe, 'X'},
    },
};
...
static bool do_detailed_fprintf_tests(bool verbose, bool passed)
{
    ...
    for (i = 0; i < sizeof(fprintf_specs) / sizeof(fprintf_specs[0]); i++)
    {
        ...
        const uint64_t *args = fprintf_specs[i].args;
        int ret;
