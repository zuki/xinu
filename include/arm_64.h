...
static inline uint64_t timerfreq(void)
{
    uint64_t f;
...
static inline uint64_t resr(void)
{
    disb();
    uint64_t r;
...
static inline uint64_t relr(void)
{
    disb();
    uint64_t r;
...
static inline void lesr(uint64_t p)
{
    disb();
    asm volatile("msr esr_el1, %[x]" : : [x]"r"(p));
    disb();
}

/* Read Fault Address Register (EL1) */
static inline uint64_t rfar(void)
{
    disb();
    uint64_t r;
