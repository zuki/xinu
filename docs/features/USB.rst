...
これはいくつかの要因、たとえば、事実上あらゆる種類のデバイスのサポート、
デバイスの動的な着脱のサポート、複数のバージョンのUSB仕様との後方互換性、
サポートされる複数の速度と転送タイプなどにより発生します。USB 2.0仕様は
650ページにも及びますが、それでも特定のUSBデバイスを制御するUSBソフトウェア
スタックとドライバをゼロから実装するために必要な情報のほんの一部しか
カバーしていません。

USBは非常に複雑なため、ここではUSBを完全に説明することはできませんし、
Embedded XinuにおけるUSBの実装を完全に説明することもできません。ここでは
Embedded Xinuの実装という文脈におけるUSBの概要を説明します。USBの完全な
詳細については必然的にUSBの仕様そのものやその他の関連する仕様やウェブ
...
同様に、素朴なユーザが「USBデバイス」と呼ぶような1つの物理的パッケージが
...
ポイントは以下のいずれかのタイプのUSB転送に関連付けられます：

- **コントロール** 転送。これは通常、デバイスのコンフィグレーションに使用されます。
  この転送には主に2つのユニークな特徴があります。第一に、コントロール転送される実際の
  データの前に常に SETUP と呼ばれる特別なパケットがUSB上で送信され、ソフトウェアは
  このパケットの内容を指定する必要があります。第二に、すべてのデバイスにはコント
  ロール転送が可能な双方向のエンドポイントが1つありますが、このエンドポイントはエンド
  ポイントディスクリプタに明示的に記載されることはありません。
...
しかし、レガシーデバイスとの後方互換性を維持する必要があるため、残念ながら、
USBソフトウェア（主にホストコントローラドライバ）は転送速度を考慮する必要が
あります。少なくとも、LSまたはFSで接続されたデバイスとの転送は、HSで動作する
USBバスの部分を大幅に減速させることなくLSまたはFSの転送を実行できるようにする
一連の **分割トランザクション** として実行されることを認識する必要があります。

これを書いている時点では、Embedded XinuのUSBサブシステムはUSB 2.0を
サポートしており、LS、FS、HSで動作するデバイスをサポートしています。
USB 3.0のSSはサポートしていません。
...
システムで使用されている方法であり、USBの設計思想に基づいたもっとも理に
...
.. note::
    必要であれば、特定のUSBデバイスドライバを ``devtab``  のデバイス
    エントリとして提供することはできます。ただし、物理デバイスは依然として
...
必要があります。たとえば、 :source:`system/platforms/arm-rpi3/usb_dwc_hcd.c`
はRaspberry Pi 3B+ハードウェアで使用されるホストコントローラ
デバイスです。

最後に、実際にサポートしたいデバイスのドライバを置くディレクトリを ``platformVars`` の
 ``DEVICES`` 変数に追加し、 ``xinu.conf`` にそのスタティック
デバイスを定義することによりサポートしたいUSBデバイスを有効にする
必要があります。たとえば、Raspberry Pi 3B+では :source:`device/lan7800` にある
Microchip LAN7800 USB Ethernetのドライバを有効にするために ``DEVICES`` に ``lan7800`` を
追加し、 ``xinu.conf`` で ``ETH0`` デバイスを定義しています。

組込みシステム用のUSB
~~~~~~~~~~~~~~~~~~~~~~~~

デバッグ機能が重要でない完全な組み込みシステムの場合、ヒューマンフレンドリ機能は
不要であり、USBコアから省略することができます。詳細については
...
APIを使って実装します。このAPIによりドライバはドライバ自身の登録、
...
最初のステップは、Xinuが対象となるハードウェアのUSBホストコントローラが
すでにサポートされているか否かを確認することです。もしされていればそのコードを
...
- Embedded Xinu USB ホストコントローラドライバ. (例: :source:`system/platforms/arm-rpi3/usb_dwc_hcd.c`)
